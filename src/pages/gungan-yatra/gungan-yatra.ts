...
/**
 * Generated class for the GunganYatraPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-gungan-yatra',
  templateUrl: 'gungan-yatra.html',
})
export class GunganYatraPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad GunganYatraPage');
        ...
        .then(data => {
          ...
          for (let i = 0; i < this.images.length; i+=2) { //iterate images
            ...
            if (this.images[i]) { //check file URI exists
              this.grid[rowNum][0] = 'http://shakambarimata.com/' + this.images[i].path + '/'+ this.images[i].banner_image //insert image
            }
        
            if (this.images[i+1]) { //repeat for the second image
              this.grid[rowNum][1] = 'http://shakambarimata.com/'+ this.images[i].path + '/' + this.images[i+1].banner_image
            }
